...
const ChatView = () => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            const messages = await getAllMessages(loggedInUser.username, selectedUser.username);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            await chatService.post('/chat/messages', { message: JSON.stringify(messageObject) });
        ...
        <Container maxWidth="lg" sx={{ p: 2 }}>
            ...
            >
                ...
                onChange={(e) => {
                    ...
                    setFilteredUsers(users.filter((user) => user.username.toLowerCase().includes(searchTerm.trim())));
                }}
                sx={{ mb: 2 }}
                />
                <List>
            ...
            <Box flex={1} display="flex" flexDirection="column">
                ...
                {!selectedUser ? (
                    ...
                    <Typography variant="body1">Click on a user name to start chatting</Typography>
                    </Box>
                ) : (
                    <>
                    {messages.map((msg, index) => (
                        ...
                        sx={{
                            display: 'flex',
                            justifyContent: msg.sender === loggedInUser?.username ? 'flex-end' : 'flex-start',
                            mb: 1,
                        }}
                        >
                        <Typography
                            sx={{
                            padding: 1,
                            backgroundColor: msg.sender === loggedInUser?.username ? '#1976d2' : '#e3f2fd',
                            color: msg.sender === loggedInUser?.username ? 'white' : 'black',
                            borderRadius: 1,
                            maxWidth: '60%',
                            wordWrap: 'break-word',
                            whiteSpace: 'pre-line',
